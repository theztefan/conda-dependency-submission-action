...
test('Uses file basename as manifest name when YAML has no name', async () => {
  const tmpFile = path.join('test', 'environment-nameless.yaml');
  const content = `channels:\n- defaults\ndependencies:\n- python=3.11\n- numpy=1.26\n`;
  fs.writeFileSync(tmpFile, content, 'utf8');
  try {
    const files = conda.searchFiles('test', 'environment-nameless.yaml');
    ...
    expect(m.file.source_location).toEqual('test/environment-nameless.yaml');
  } finally {
